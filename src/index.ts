...
async function ragSearch(query: string, limit: number, env: Env): Promise<string> {
  ...
  } catch (e) { console.error('[RAG] corpus retrieval failed (embed/vectorize):', (e as Error).message); return ''; }
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    return err(`Unknown endpoint: ${path}`, 404);
      ...
      // Without this, any thrown error (LLM exhausted, embed quota, D1 fault) became a
      // bare 500 with no CORS headers — which the browser surfaces as an opaque "load
      // failed." Now the UI gets the actual reason, and `wrangler tail` logs it.
      console.error('[FETCH] unhandled error:', (e as Error).message);
      return err('Worker error: ' + ((e as Error).message || 'unknown'), 500);
    }
  },
